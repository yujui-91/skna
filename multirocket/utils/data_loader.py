...
def read_univariate_ucr_skna(filename, pattern, normalise=True):
    # 获取所有符合条件的文件路径
    """
    ...
    # 根据文件名中的数字对路径进行排序
    input_paths_sorted = sorted(input_paths, key=extract_number_from_path)
    # 划分健康人和病人的文件路径
    midpoint = len(input_paths) // 2
    # 假设前200个是健康人
    healthy_paths = input_paths_sorted[:midpoint]  # 假设前200个是健康人
    patient_paths = input_paths_sorted[midpoint:]  # 假设后200个是病人

    # 定义分割函数，比例划分训练集和测试集
    def split_data(paths, train_ratio=0.85):
        split_point = int(len(paths) * train_ratio)
        return paths[:split_point], paths[split_point:]

    # 分割健康人和病人的训练集和测试集
    train_paths_healthy, test_paths_healthy = split_data(healthy_paths)
    train_paths_patient, test_paths_patient = split_data(patient_paths)

    # 合并训练集和测试集
    train_paths = train_paths_healthy + train_paths_patient
    test_paths = test_paths_healthy + test_paths_patient
    ...
    merged_train_data = load_and_merge_data(train_paths)
    merged_test_data = load_and_merge_data(test_paths)
    ...
    X_train, y_train = data_load(merged_train_data)
    X_test, y_test = data_load(merged_test_data)

    return X_train, y_train, X_test, y_test


def read_univariate_ucr_bladder(filename, pattern, normalise=True):
    # 获取所有符合条件的文件路径
    """
            Load data from a .tsv file into a Pandas DataFrame.
        """
    ...
    input_paths_sorted = sorted(input_paths, key=extract_number_from_path)
    # 划分健康人和病人的文件路径
    midpoint = 166 #前34人為健康人 改
    healthy_paths = input_paths_sorted[:midpoint]  # 假设前34个是健康人 166是健康人
    patient_paths = input_paths_sorted[midpoint:]  # 假设后51个是病人 21是病人

    # 定义分割函数，按照80%和20%的比例划分训练集和测试集
    def split_data(paths, train_ratio=0.8):
        split_point = round(len(paths) * train_ratio)
        return paths[:split_point], paths[split_point:]
...
def load_train_test_folder(filename, pattern, normalise=True):
    ...
    """
        ...
        """
    data_paths = glob.glob(os.path.join(filename, '*'))  # list of all paths
    input_paths = [p for p in data_paths if os.path.isfile(p) and p.endswith('.tsv')]  # 讀取檔名沒有「skna」的資料
    ...
    def load_data(paths):
        ...
        for filename in paths:
            # 使用np.loadtxt读取每个文件的数据
            # data = np.loadtxt(filename, delimiter='\t')
            df = pd.read_csv(filename, delimiter='\t', header=None).astype(np.float32)  # 無header
            num_rows = df.shape[0]
            # 将读取的数据添加到列表中
            data_list.append(df)
            window_per_person.append(num_rows)
        ...
        return total_data, window_per_person


    total_data, window_per_person = load_data(input_paths)
