...
parser.add_argument("-p", "--problem", type=str, required=False, default="sr10000_10s_90pct_logistic_51_41") # output data path
...
if __name__ == '__main__':
    ...
    hp_path = r"V:\dunwei\MACE\dataset\SKNA_spectrum_signal\ch1\sr10000_10s_90pct/"
    ...
    if os.path.exists(data_folder):
        ...
        train_cm = []
        test_cm = []
        print(len(h_file_list), len(p_file_list))
        for i in tqdm(range((int((p.shape[0])/1))), desc="iteration"): # if divided window need /141
            ...
            st_h = offset_h_data
            ed_h = st_h + (h_size) # if divided window need *141
            st_p = offset_p_data
            ed_p = st_p + (p_size) # if divided window need *141

            st_h_f = offset_h_file
            ed_h_f = st_h_f + h_size
            st_p_f = offset_p_file
            ed_p_f = st_p_f + p_size

            offset_h_data = ed_h
            offset_p_data = ed_p
            offset_h_file = ed_h_f
            offset_p_file = ed_p_f
