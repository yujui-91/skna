...
parser.add_argument("-p", "--problem", type=str, required=False, default="sr10000_1000_2000_20s_logistic_longer_51_41_100pts_20win") # output data path ##要換路徑
...
if __name__ == '__main__':
    ...
    output_path = r"D:\M143020071\multirocket_result/" ###要換路徑
    classifier_name = "MultiRocket_{}".format(num_features)
    hp_path = r"D:\M143020071\raw_data_result\SKNA_signal\ch1\sr10000_1000_2000_20s_ECG_signal_rpeak_5_10min_longer_100pts_mr_20win/" ###要換路徑
    ...
    if os.path.exists(data_folder):
        ...
        X_all_trans = np.load(data_folder + "x_transform_all.npy").astype(np.float32)
        y_all_trans = np.load(data_folder + "y_transform_all.npy")

        # 假設 label 0 是 Healthy (non_mace), 1 是 Patient (mace)
        # 透過 boolean indexing 拆分資料
        h = X_all_trans[y_all_trans == 0]
        p = X_all_trans[y_all_trans == 1]

        print(f"Loaded and split data. Healthy shape: {h.shape}, Patient shape: {p.shape}")
        ...
        for i in tqdm(range((int((len(p_file_list))/1))), desc="iteration"):
            ...
            #X_train = X_train.reshape((X_train.shape[0], X_train.shape[1], 1))  多餘
            #X_test = X_test.reshape((X_test.shape[0], X_test.shape[1], 1)) 多餘

            encoder = LabelEncoder()
            y_train = encoder.fit_transform(y_train) #浮點數轉成整數 訓練+轉換          
            y_test = encoder.transform(y_test)#用剛學規則轉換
            ...
            #如果現在是在跑標準的學術資料集（且不是第一輪），請放棄剛剛辛苦切好的資料，改用「官方指定」的切法來切。
    ...
    #### 為了顯示原本的預測值做的修改
            if yhat_train is not None:
                data_yhat_train_original = {
                    "yhat_train_original": yhat_train.ravel() #預測機率大多為二維 為了做df 所以需要攤平變成[0.1, 0.9, 0.3]
                }   #excel的標籤^^^^^^     內容^^^^^^^
    ...
    ####    把最好的閥值、MCC記錄下來
            ...
            #將模型對「測試集 (Test Set)」預測出來的「原始機率值」保存成 CSV 檔案
            ...
            #拿剛剛在「模擬考 (Train)」學到的「最佳標準 (Threshold)」，
            # 正式應用到「期末考 (Test)」上，算出最終成績
