...
@njit("float32[:](float32[:,:],int32[:],int32[:],float32[:])", # FIXME: float64改float32做測試
...
@njit(
    "float32[:,:](float32[:,:],float32[:,:],Tuple((int32[:],int32[:],float32[:])),Tuple((int32[:],int32[:],float32[:])),int32)", # FIXME: float64改float32做測試
...
class MultiRocket:
    ...
    def __init__(
        ...
        self.diff1_parameters = None    # 一階微分數據的卷積參數

        self.n_features_per_kernel = 4  # 每個卷積核會產生 4 個特徵 (PPV, Max, Mean, etc.)
        ...
        self.apply_kernel_on_test_duration = 0   #計時器初始化
    ...
    def fit(self, x_train, y_train, predict_on_train=True):
        ...
        self.generate_kernel_duration = 0
        self.apply_kernel_on_train_duration = 0
        self.train_transforms_duration = 0  #計時器初始化

        start_time = time.perf_counter() #總計時
        ...
        # --- 步驟 2: 生成卷積核參數 (Generate Kernels) ---
        #隨機生成 Kernel 的長度、權重、Bias、Dilation 等參數
        _start_time = time.perf_counter()
        self.base_parameters = fit(   #此處fit是外部函式(Numba)
            x_train,
        ...
        y_train_2D = y_train.reshape(-1, 1)  # (-1表示自動計算該維度(row)大小, 指定該維度(column)數量1)
        x_train_transform_label = np.hstack((y_train_2D, x_train_transform), dtype=np.float32) 
        ...
        if self.clf.lower() == "ridge":
            self.classifier = make_pipeline(
                StandardScaler(),
                RidgeClassifierCV( #自動交叉驗證尋找最佳的正則化參數 嘗試不同alpha值
        ...
        _start_time = time.perf_counter()
        # self.classifier.fit(x_train_transform, y_train) # FIXME: 原本沒有註解
        ...
        return yhat, x_train_transform_label
    ...
    def predict(self, x):
        ...
        self.test_transforms_duration = 0 #計時器初始化 
        ...
        x_transform = transform(
            ...
            self.base_parameters, self.diff1_parameters, 
            self.n_features_per_kernel
        )# 這些是在 fit() 階段生成的參數。確保測試集使用與訓練集相同的邏輯提取特徵。
        ...
        return yhat, x_transform

    # FIXME: add transform only function 
    def transform(self, x):
        '''
            Add new function to just transform data without predicting 2025-12-14
        '''
        if self.verbose > 1:
