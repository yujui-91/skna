...
parser.add_argument("-p", "--problem", type=str, required=False, default="sr2500_500_1000_logistic") # output data path
...
if __name__ == '__main__':
    ...
    if os.path.exists(data_folder):
        ...
        train_cm = []
        test_cm = []
        for i in tqdm(range((int((p.shape[0]/141)/5))), desc="iteration"):
            st_h = 23 * i * 141
            ed_h = st_h + (23 * 141)
            st_p = 5 * i * 141
            ed_p = st_p + (5 * 141)

            st_h_f = 23 * i # f = filename
            ed_h_f = st_h_f + 23
            st_p_f = 5 * i
            ed_p_f = st_p_f + 5
